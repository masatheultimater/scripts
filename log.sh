...
from lib.houjinzei_common import read_frontmatter
...
for md_path in sorted(root.rglob("*.md")):
    ...
    try:
        ...
        continue
    topic = fm.get("topic")
    if isinstance(topic, str) and topic.strip() and keyword in topic.strip():
        print(f"{md_path}\t{topic.strip()}")
PY
)
...
fm_data, body = read_frontmatter(path)

calc_correct = to_int(fm_data.get("calc_correct", 0))
calc_wrong = to_int(fm_data.get("calc_wrong", 0))
kome_total = to_int(fm_data.get("kome_total", 0))
...
if current_status == "卒業":
    ...
    stage = compute_stage(current_status, kome_total, calc_correct, calc_wrong)
    fm_data["stage"] = stage
    if current_status == "未着手" and stage != "未着手":
        fm_data["status"] = "学習中"
...
write_frontmatter(path, fm_data, body)
