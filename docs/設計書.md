...
interval_index: 2                # 間隔反復インデックス（0=初回, 1=3日後完了, 2=7日後完了, 3=14日後完了, 4=28日後完了=卒業）
                                 #   正解時にインクリメント、不正解時に0リセット
...
status: "学習中"                 # ライフサイクル状態（komekome_writeback.sh が更新）
                                 #   値: 未着手 / 学習中 / 復習中 / 卒業
                                 #   卒業条件（優先）: interval_index>=4 + kome_total>=4 + 正解
                                 #   卒業条件（レガシー）: status="復習中" + gap≥25日 + kome_total≥4 + 正解
...
| interval_index | komekome_writeback.sh | 正解時+1、不正解時0リセット。>=4で卒業候補 |
...
| `enrich_topics.sh` | 空テンプレートノートの本文充実（claude -p） | 手動 |
| `cron_wrapper.sh` | cron用ログ付きラッパー（失敗時Windows通知） | cron経由 |
| `catchup.sh` | WSL起動時の未実行ジョブ補完 | @reboot |
| `ingest.sh` | PDF取り込みパイプライン | 手動 |
| `stage2.sh` | STAGE 2 ノート生成 | ingest.sh経由 |
| `test_e2e.sh` | E2Eシミュレーションテスト（7テスト） | 手動 |
...
GRADUATION_GAP_DAYS = 25    # 卒業判定（レガシー）: 最終演習からの経過日数
GRADUATION_MIN_KOME = 4     # 卒業判定: 最低累積コメ数
KOME_THRESHOLD_REVIEW = 16  # kome_total >= 16 で stage=復習中
INTERVAL_DAYS = (3, 7, 14, 28)     # 間隔反復スケジュール（日数）
GRADUATION_INTERVAL_INDEX = 4      # interval_index >= 4 で卒業候補

# 関数
compute_stage(status, kome_total, calc_correct, calc_wrong) → str  # stage 算出
normalize_stage(raw_stage, status) → str                           # stage 正規化（集計用）
next_review_days(interval_index) → int                             # 次回復習までの日数
is_graduation_ready(interval_index, kome_total) → bool             # 卒業条件判定
...
- [x] ~~コメコメアプリ側の `intervalIndex` フィールド追加~~ → 実装済み。generate_quiz.sh が intervalIndex を出力、komekome_writeback.sh が interval_index ベースの卒業判定を実行
- [x] ~~論点ノートの本文充実~~ → enrich_topics.sh 実装済み。`bash enrich_topics.sh` で claude -p による自動生成を実行可能。notebooklm_export.sh の has_substantive_content() バグも修正済み
